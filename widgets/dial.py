...
class ValueDial(qtw.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dial = qtw.QDial(self, **dialArgs)
  ...
  qproperty-arcWidth: 3; 
    ...
    def updateSize(self):
        ...
        self.offset = max(minWidth, fm.height()) / 1
    ...
    def translateMouseEvent(self, event):
        ...
        return qtg.QMouseEvent(event.type(),
                               self.dial.mapFrom(self, event.pos()),
                               event.button(), event.buttons(), event.modifiers())

    def changeEvent(self, event):
        if event.type() == qtc.QEvent.FontChange:
            self.updateSize()
    ...
    def paintEvent(self, event):
        ...
        reference = qtc.QLineF.fromPolar(radius, 0).translated(self.rect().center())
        ...
        for p in range(0, fullRange + rangeOffset, self.notchSize()):
            ...
            textRect.setSize(fm.size(qtc.Qt.TextSingleLine, str(value)))
            textRect.moveCenter(reference.p2().toPoint())
            qp.drawText(textRect, qtc.Qt.AlignCenter, str(value))
